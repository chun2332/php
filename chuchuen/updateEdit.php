...
if( $_FILES["proImg"]["error"] === 0 ) {
    ...
    if( move_uploaded_file($_FILES["proImg"]["tmp_name"], "./images/".$studentImg) ) {
        /**
         * 刪除先前的舊檔案: 
        ...
        $sqlGetImg = "SELECT `proImg` FROM `product` WHERE `id` = ? ";
        ...
        if($stmtGetImg->rowCount() > 0) {
            ...
            if($arrImg['proImg'] !== NULL){
                // 刪除實體檔案
                @unlink("./images/".$arrImg['proImg']);
            } 
